...
#if JUCE_GLOBAL_HOTKEYS_ENABLE_CARBON_API ||                                   \
    JUCE_GLOBAL_HOTKEYS_ENABLE_CGEVENTTAP_API
  return true;
#else
  return false;
#endif
...
bool GlobalHotKey::PlatformSpecificData::requestPermissions() {
  ...
  DBG("GlobalHotKey: Permissions not available, attempting to trigger "
      "permission dialog");

  // For CGEventTap, we need to trigger the system permission dialog
  // This is done by attempting to create an event tap
  static auto dummyCallback = [](CGEventTapProxy, CGEventType, CGEventRef event,
                                 void *) -> CGEventRef { return event; };

  auto testTap = CGEventTapCreate(
      kCGSessionEventTap, kCGHeadInsertEventTap, kCGEventTapOptionDefault,
      CGEventMaskBit(kCGEventKeyDown), dummyCallback, nullptr);
...
bool GlobalHotKey::PlatformSpecificData::hasPermissions() {
  ...
  static auto testCallback = [](CGEventTapProxy, CGEventType, CGEventRef event,
                                void *) -> CGEventRef {
...
bool GlobalHotKey::PlatformSpecificData::registerWithCarbon(
    const KeyCode &keyCode, const ModifierKeys &modifiers) {
  ...
  OSStatus status =
      RegisterEventHotKey(carbonKey, carbonModifiers, carbonHotKeyID,
                          GetApplicationEventTarget(), 0, &carbonHotKeyRef);

  if (status != noErr) {
    DBG("GlobalHotKey: RegisterEventHotKey failed with status: " +
        String(status));
    return false;
  } else {
  ...
  if (carbonHotkeys.empty()) {
    ...
    status =
        InstallApplicationEventHandler(&globalCarbonEventHandler, 1, &eventType,
                                       nullptr, &globalCarbonEventHandlerRef);

    if (status != noErr) {
      DBG("GlobalHotKey: InstallApplicationEventHandler failed with status: " +
          String(status));
...
OSStatus GlobalHotKey::PlatformSpecificData::globalCarbonEventHandler(
    EventHandlerCallRef nextHandler, EventRef event, void *userData) {
#if JUCE_GLOBAL_HOTKEYS_DEBUG_LOGGING
  DBG("GlobalHotKey: Global Carbon event handler called - Class: " +
      String(GetEventClass(event)) + ", Kind: " + String(GetEventKind(event)));
#endif
...
#if JUCE_GLOBAL_HOTKEYS_DEBUG_LOGGING
    DBG("GlobalHotKey: Global Carbon hotkey event - GetEventParameter "
        "status: " +
        String(status) + ", Signature: " + String(hotKeyID.signature) +
        ", ID: " + String(hotKeyID.id));
#endif

    if (status == noErr && hotKeyID.signature == 'JUCE') {
      // Find the corresponding hotkey instance by searching through all
      // registered hotkeys
      for (auto &pair : carbonHotkeys) {
        if (pair.first->signature == hotKeyID.signature &&
            pair.first->id == hotKeyID.id) {
          ...
          MessageManager::callAsync(
              [hotkeyData]() { hotkeyData->owner.triggerCallback(); });
          return noErr;
        }
      }
#if JUCE_GLOBAL_HOTKEYS_DEBUG_LOGGING
      DBG("GlobalHotKey: No matching hotkey found for ID: " +
          String(hotKeyID.id));
#endif
    }
  }
...
UInt32 GlobalHotKey::PlatformSpecificData::convertKeyCodeToCarbon(
  ...
  if (juceKey >= 'A' && juceKey <= 'Z') {
    // Use the correct virtual key code mapping from
    // translateVirtualToAsciiKeyCode
    switch (juceKey) {
    case 'A':
    ...
    case 'D':
      return 0x02; // kVK_ANSI_D
    case 'F':
      return 0x03; // kVK_ANSI_F
    case 'H':
      return 0x04; // kVK_ANSI_H
    case 'G':
      return 0x05; // kVK_ANSI_G  ← 이것이 핵심!
    case 'Z':
      return 0x06; // kVK_ANSI_Z
    case 'X':
      return 0x07; // kVK_ANSI_X
    case 'C':
    ...
    case 'Q':
      return 0x0C; // kVK_ANSI_Q
    case 'W':
      return 0x0D; // kVK_ANSI_W
    case 'E':
      return 0x0E; // kVK_ANSI_E
    case 'R':
      return 0x0F; // kVK_ANSI_R
    case 'Y':
      return 0x10; // kVK_ANSI_Y
    case 'T':
      return 0x11; // kVK_ANSI_T
    case 'O':
      return 0x1F; // kVK_ANSI_O
    case 'U':
      return 0x20; // kVK_ANSI_U
    case 'I':
      return 0x22; // kVK_ANSI_I
    case 'P':
      return 0x23; // kVK_ANSI_P
    case 'L':
      return 0x25; // kVK_ANSI_L
    case 'J':
      return 0x26; // kVK_ANSI_J
    case 'K':
      return 0x28; // kVK_ANSI_K
    case 'N':
      return 0x2D; // kVK_ANSI_N
    case 'M':
      return 0x2E; // kVK_ANSI_M
    default:
      return 0;
    }
  }
  if (juceKey >= '0' && juceKey <= '9') {
    ...
    switch (juceKey) {
    case '1':
      return 0x12; // kVK_ANSI_1
    case '2':
      return 0x13; // kVK_ANSI_2
    case '3':
      return 0x14; // kVK_ANSI_3
    case '4':
      return 0x15; // kVK_ANSI_4
    case '6':
      return 0x16; // kVK_ANSI_6
    case '5':
      return 0x17; // kVK_ANSI_5
    case '9':
      return 0x19; // kVK_ANSI_9
    case '7':
      return 0x1A; // kVK_ANSI_7
    case '8':
      return 0x1C; // kVK_ANSI_8
    case '0':
      return 0x1D; // kVK_ANSI_0
    default:
      return 0;
    }
  }
  ...
  // Punctuation marks
  case '[':
    return 0x21; // kVK_ANSI_LeftBracket
  case ']':
    return 0x1E; // kVK_ANSI_RightBracket
  case ';':
    return 0x29; // kVK_ANSI_Semicolon
  case '\'':
    return 0x27; // kVK_ANSI_Quote
  case ',':
    return 0x2B; // kVK_ANSI_Comma
  case '.':
    return 0x2F; // kVK_ANSI_Period
  case '/':
    return 0x2C; // kVK_ANSI_Slash
  case '\\':
    return 0x2A; // kVK_ANSI_Backslash
  case '`':
    return 0x32; // kVK_ANSI_Grave
  case '-':
    return 0x1B; // kVK_ANSI_Minus
  case '=':
  ...
  case 0x60000:
    return 0x52; // kVK_ANSI_Keypad0
  case 0x60001:
    return 0x53; // kVK_ANSI_Keypad1
  case 0x60002:
    return 0x54; // kVK_ANSI_Keypad2
  case 0x60003:
    return 0x55; // kVK_ANSI_Keypad3
  case 0x60004:
    return 0x56; // kVK_ANSI_Keypad4
  case 0x60005:
    return 0x57; // kVK_ANSI_Keypad5
  case 0x60006:
    return 0x58; // kVK_ANSI_Keypad6
  case 0x60007:
    return 0x59; // kVK_ANSI_Keypad7
  case 0x60008:
    return 0x5B; // kVK_ANSI_Keypad8
  case 0x60009:
    return 0x5C; // kVK_ANSI_Keypad9

  // Special keys
  case ' ':
    return kVK_Space; // spaceKey
  case 0x1000d:
    return kVK_Return; // returnKey
  case 0x1001b:
    return kVK_Escape; // escapeKey
  case 0x10008:
    return kVK_Delete; // backspaceKey
  case 0x1007f:
    return kVK_ForwardDelete; // deleteKey
  case 0x10009:
    return kVK_Tab; // tabKey
  case 0x10012:
    return kVK_LeftArrow; // leftKey
  case 0x10014:
    return kVK_RightArrow; // rightKey
  case 0x10013:
    return kVK_UpArrow; // upKey
  case 0x10015:
    return kVK_DownArrow; // downKey
  case 0x10010:
    return kVK_Home; // homeKey
  case 0x10011:
    return kVK_End; // endKey
  case 0x10016:
    return kVK_PageUp; // pageUpKey
  case 0x10017:
    return kVK_PageDown; // pageDownKey
  default:
    return 0;
  }
}

UInt32 GlobalHotKey::PlatformSpecificData::convertModifiersToCarbon(
...
#if JUCE_GLOBAL_HOTKEYS_DEBUG_LOGGING
  DBG("GlobalHotKey: Modifier conversion - Command: " +
      String(modifiers.isCommandDown() ? "Yes" : "No") +
...
bool GlobalHotKey::PlatformSpecificData::registerWithCGEventTap(
    const KeyCode &keyCode, const ModifierKeys &modifiers) {
  ...
  if (eventTap == nullptr) {
    DBG("GlobalHotKey: CGEventTapCreate failed - possibly due to lack of Input "
...
CGEventRef GlobalHotKey::PlatformSpecificData::eventTapCallback(
    CGEventTapProxy proxy, CGEventType type, CGEventRef event, void *userData) {
  ...
  if (type == kCGEventKeyDown) {
    ...
    if (self->matchesRegisteredHotkey(event)) {
      ...
      // Forward to JUCE message thread
      MessageManager::callAsync([self]() { self->owner.triggerCallback(); });
...
bool GlobalHotKey::PlatformSpecificData::matchesRegisteredHotkey(
...
CGKeyCode
GlobalHotKey::PlatformSpecificData::convertKeyCodeToCG(const KeyCode &keyCode) {
  // Same mapping as Carbon since CGKeyCode uses the same values
  return convertKeyCodeToCarbon(keyCode);
}

CGEventFlags GlobalHotKey::PlatformSpecificData::convertModifiersToCG(
    const ModifierKeys &modifiers) {
...
#if JUCE_GLOBAL_HOTKEYS_DEBUG_LOGGING
  DBG("GlobalHotKey: CGEventTap modifier conversion - Command: " +
      String(modifiers.isCommandDown() ? "Yes" : "No") +
      ", Shift: " + String(modifiers.isShiftDown() ? "Yes" : "No") +
      ", Alt: " + String(modifiers.isAltDown() ? "Yes" : "No") +
      ", Ctrl: " + String(modifiers.isCtrlDown() ? "Yes" : "No") +
